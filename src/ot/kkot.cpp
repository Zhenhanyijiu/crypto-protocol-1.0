...
int kkot_sender::send(conn* sock, const vector<vector<uint8_t>>& data, int N,
                      int bit_l) {
  assert(bit_l > 0);
  assert(bit_l <= 8);
...
int kkot_sender::send(conn* sock, const vector<vector<uint64_t>>& data, int N,
                      int bit_l) {
  assert(bit_l > 8);
  assert(bit_l <= 64);
  int num_otext = data.size();
  ...
  for (size_t i = 0; i < num_otext; i++) {
    memset(tmp, 0, min_num);
    for (size_t j = 0; j < N; j++) {
      uint64_t key = (*(uint64_t*)&out_masks[i][j]) & mask;
      uint64_t x = data[i][j] ^ key;
      int bit_pos = j * bit_l, shift = 0;
      for (size_t k = bit_pos; k < bit_pos + bit_l; k++, shift++) {
        int byte_index = k / 8, bit_index = k % 8;
        tmp[byte_index] |= ((x >> shift) & 0x1) << bit_index;
  ...
  sock->send(ssbuff.str());
  cout << "recv mask:" << (uint64_t)mask << endl;
...
int kkot_receiver::recv(conn* sock, const vector<int>& r_i,
                        vector<uint64_t>& out_data, int N, int bit_l) {
  assert(bit_l > 8);
  assert(bit_l <= 64);
  int numOTExt = r_i.size();
  ...
  for (size_t i = 0; i < numOTExt; i++, offset += min_num) {
    memcpy(tmp, buff.data() + offset, min_num);
    uint64_t key = (*(uint64_t*)&out_masks[i]) & mask;
    int bit_pos = r_i[i] * bit_l, shift = 0;
    int byte_index = 0, bit_index = 0;
    for (size_t k = bit_pos; k < bit_pos + bit_l; k++, shift++) {
      byte_index = k / 8, bit_index = k % 8;
      out_data[i] |= uint64_t((tmp[byte_index] >> bit_index) & 0x1) << shift;
    }
    out_data[i] ^= key;
  }
  cout << "recv mask:" << (uint32_t)mask << endl;
