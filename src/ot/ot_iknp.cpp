...
int IknpOtExtReceiver::genPublicParamFromNpot(u8 **pubParamBuf,
...
int IknpOtExtReceiver::genRecoverMsg(const BitVector &choicesWidthInput,
  ...
  for (u64 superBlkIdx = 0; superBlkIdx < numSuperBlocks; ++superBlkIdx) {
    ...
    if (uIter == uEnd) {
      ...
      memcpy(uBuffOutputAll.data() + offset, uBuffOutput.data(),
             uBuffOutput.size() * 16);
      // uBuffOutput.data();
...
int IknpOtExtSender::genPK0FromNpot(u8 *pubParamBuf,
                                    const u64 pubParamBufByteSize, u8 **pk0Buf,
                                    u64 *pk0BufSize) {
...
//     return 0;
// }
int IknpOtExtSender::getEncMsg(const vector<block> &uBuffInputAll,
  ...
  for (u64 superBlkIdx = 0; superBlkIdx < numSuperBlocks; ++superBlkIdx) {
    ...
    if (uIter == uEnd) {
      ...
      memcpy(uBuffInput.data(), uBuffInputAll.data() + offset,
    ...
    //             (messages.data() + messages.size()) - mEnd); for (u64 rowIdx
    //             = doneIdx,
    //                      j = 0;
    //                  j < xx; ++rowIdx, ++j)
    //             {
    //                 if (neq(((block *)u.data())[j],
    //                 messages[rowIdx][choice[j]]))
    //                 {
