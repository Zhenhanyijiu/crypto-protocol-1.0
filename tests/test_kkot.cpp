...
void test_kkot_sender_uint64(int num_ot, vector<vector<uint64_t>>& in_data,
                             config_param& param, int N, int bit_l) {
  int numOTExt = num_ot;
  connection c(0, "127.0.0.1", 9001);
  kkot_sender kkot(param);
...
void test_kkot_receiver_uint64(const vector<int>& r_i,
                               vector<uint64_t>& out_data, config_param& param,
                               int N, int bit_l) {
  //   int numOTExt = choices.size();
  connection c(1, "127.0.0.1", 9001);
  kkot_receiver kkot(param);
...
int main_test_uint64(int argc, char** argv) {
  srand(time(NULL));
  bool is_power = false;
  int num_Ote = 1 << 19;
  if (argc > 1) {
  ...
  config_param param;
  param.hasher_name = "blake3";
  uint64_t mask = get_mask(bit_l);
  printf("===== mask:%llx\n", mask);
  vector<vector<uint64_t>> inputs(num_Ote);
  for (size_t i = 0; i < num_Ote; i++) {
    for (size_t j = 0; j < N; j++) {
  ...
  cout << "========== start time " << tp.get_time_piont_ms() << " ms" << endl;
  //   vector<vector<block>> out_masks;
  thread th1(test_kkot_sender_uint64, num_Ote, ref(inputs), ref(param), N,
             bit_l);

  vector<uint64_t> out_data;
  thread th2(test_kkot_receiver_uint64, ref(choices), ref(out_data), ref(param),
             N, bit_l);
  th1.join();
  th2.join();
  cout << "========== end time " << tp.get_time_piont_ms() << " ms" << endl;
  ...
  for (size_t i = 0; i < num_Ote; i++) {
    if (i < 3 && i < num_Ote) {
      ...
      //   set<block> tmp;
      for (size_t j = 0; j < N; j++) {
        cout << "[" << j << "]" << uint64_t(inputs[i][j]) << endl;
        // tmp.insert(out_masks[i][j]);
      }
      //   if (tmp.size() != N) {
      //     cout << "tmp.size()!=N error i:" << i << endl;
      //     return 0;
      //   }
      cout << "[" << (uint64_t)choices[i] << "]" << uint64_t(out_data[i])
           << endl;
      cout << "[" << (uint64_t)choices[i] << "]"
