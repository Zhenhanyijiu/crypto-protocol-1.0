...
namespace osuCrypto
{
    ...
    public:
        ...
        void ecbEncNBlocks(const block *plaintext, block *ciphertext) const
        {
            ...
            for (int i = 0; i < N; ++i)
                ciphertext[i] = _mm_xor_si128(plaintext[i], mAESs[i].mRoundKey[0]);
            for (int i = 0; i < N; ++i)
                ciphertext[i] = _mm_aesenc_si128(ciphertext[i], mAESs[i].mRoundKey[1]);
            for (int i = 0; i < N; ++i)
                ciphertext[i] = _mm_aesenc_si128(ciphertext[i], mAESs[i].mRoundKey[2]);
            for (int i = 0; i < N; ++i)
                ciphertext[i] = _mm_aesenc_si128(ciphertext[i], mAESs[i].mRoundKey[3]);
            for (int i = 0; i < N; ++i)
                ciphertext[i] = _mm_aesenc_si128(ciphertext[i], mAESs[i].mRoundKey[4]);
            for (int i = 0; i < N; ++i)
                ciphertext[i] = _mm_aesenc_si128(ciphertext[i], mAESs[i].mRoundKey[5]);
            for (int i = 0; i < N; ++i)
                ciphertext[i] = _mm_aesenc_si128(ciphertext[i], mAESs[i].mRoundKey[6]);
            for (int i = 0; i < N; ++i)
                ciphertext[i] = _mm_aesenc_si128(ciphertext[i], mAESs[i].mRoundKey[7]);
            for (int i = 0; i < N; ++i)
                ciphertext[i] = _mm_aesenc_si128(ciphertext[i], mAESs[i].mRoundKey[8]);
            for (int i = 0; i < N; ++i)
                ciphertext[i] = _mm_aesenc_si128(ciphertext[i], mAESs[i].mRoundKey[9]);
            for (int i = 0; i < N; ++i)
                ciphertext[i] = _mm_aesenclast_si128(ciphertext[i], mAESs[i].mRoundKey[10]);
        }
